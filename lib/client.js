window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region styles
		const css = ".dsh-pc-pet-wrap{position:fixed;pointer-events:none;z-index:2147483000;}"
			+ ".dsh-pc-pet{pointer-events:auto;cursor:grab;user-select:none;touch-action:none;image-rendering:pixelated;}"
			+ ".dsh-pc-pet.dragging{cursor:grabbing;}"
			+ ".dsh-pc-through .dsh-pc-pet{pointer-events:none;}"
			+ ".dsh-pc-dialog{position:fixed;pointer-events:auto;background:rgba(255,255,255,.72);-webkit-backdrop-filter:blur(8px);backdrop-filter:blur(8px);color:#1f2328;border:1px solid rgba(0,0,0,.08);border-radius:10px;box-shadow:0 2px 14px rgba(0,0,0,.22);padding:6px 12px;width:240px;z-index:2147483002;display:flex;align-items:center;gap:10px;font:12px/1.45 ui-sans-serif,system-ui,sans-serif;text-align:left;cursor:pointer;box-sizing:border-box;}"
			+ ".dsh-pc-dialog.collapsed{padding:5px 12px;cursor:pointer;}"
			+ ".dsh-pc-dialog-body{flex:1;min-width:0;}"
			+ ".dsh-pc-dialog-project{font-weight:700;font-size:12.5px;color:#1f2328;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
			+ ".dsh-pc-dialog-doing{color:#6b7280;font-size:11.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px;}"
			+ ".dsh-pc-dialog-status{flex:none;font-size:16px;font-weight:800;line-height:1;}"
			+ ".dsh-pc-dialog-status.ok{color:#16a34a;}"
			+ ".dsh-pc-dialog-status.err{color:#dc2626;}"
			+ ".dsh-pc-dialog-stop{flex:none;width:24px;height:24px;border-radius:50%;border:1px solid rgba(220,38,38,.35);background:rgba(220,38,38,.1);color:#dc2626;cursor:pointer;font-size:13px;line-height:1;display:flex;align-items:center;justify-content:center;padding:0;}"
			+ ".dsh-pc-dialog-stop:hover{background:rgba(220,38,38,.28);}"
			+ ".dsh-pc-summon{position:fixed;right:18px;bottom:14px;z-index:2147483000;pointer-events:auto;cursor:pointer;background:rgba(18,20,26,.92);color:#9ecbff;border:1px solid rgba(255,255,255,.16);border-radius:14px;padding:8px 14px;font:13px ui-sans-serif,system-ui,sans-serif;box-shadow:0 6px 20px rgba(0,0,0,.4);}"
			+ ".dsh-pc-summon:hover{background:rgba(40,44,56,.95);}"
			+ ".dsh-pc-menu{position:fixed;z-index:2147483001;background:rgba(18,20,26,.95);color:#e9eaee;border:1px solid rgba(255,255,255,.16);border-radius:10px;padding:6px;font:12px/1.5 ui-sans-serif,system-ui,sans-serif;box-shadow:0 8px 28px rgba(0,0,0,.4);pointer-events:auto;min-width:172px;}"
			+ ".dsh-pc-menu .dsh-pc-title{padding:4px 10px;font-weight:600;color:#9ecbff;border-bottom:1px solid rgba(255,255,255,.12);margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
			+ ".dsh-pc-menu button{display:block;width:100%;text-align:left;background:none;border:none;color:inherit;padding:6px 10px;border-radius:6px;cursor:pointer;font:inherit;}"
			+ ".dsh-pc-menu button:hover{background:rgba(255,255,255,.1);}"
			+ ".dsh-pc-panel{font:13px/1.6 ui-sans-serif,system-ui,sans-serif;padding:4px 2px;}"
			+ ".dsh-pc-panel .dsh-pc-title{font-weight:600;margin-bottom:6px;}"
			+ ".dsh-pc-panel .row{margin:6px 0;display:flex;align-items:center;gap:8px;flex-wrap:wrap;}"
			+ ".dsh-pc-panel button{background:rgba(127,127,127,.16);border:1px solid rgba(127,127,127,.3);color:inherit;border-radius:8px;padding:5px 12px;cursor:pointer;font:inherit;}"
			+ ".dsh-pc-panel button:hover{background:rgba(127,127,127,.28);}"
			+ ".dsh-pc-panel .pets{display:flex;flex-direction:column;gap:4px;margin:6px 0;}"
			+ ".dsh-pc-panel .pets label{cursor:pointer;}"
			+ ".dsh-pc-panel .desc{opacity:.6;margin-left:2px;}"
			+ ".dsh-pc-panel .muted{opacity:.55;font-size:12px;}";
		const tagId = "dsh-codex-petcenter/css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-codex-petcenter";
		...
		async function rpc(path, body) {
			const res = await fetch('/petcenter/api/' + path, body === undefined ? {} : {
				method: 'POST',
		...
		const store = {
			...
			get() {
				return {
					...
					catalog: this.catalog, catalogTags: this.catalogTags, catalogSources: this.catalogSources,
					catalogTotal: this.catalogTotal, tagFilter: this.tagFilter,
					search: this.search, installing: this.installing
				}
			},
			set(patch) {
		...
		function PetOverlay(props) {
			...
			react.useEffect(() => {
				...
				;(s.sessions || []).forEach((ss) => {
					...
					// Only resurrect a dismissed bubble when the session went
					// from a settled state (idle/done/failed) into an ACTIVE one
					// — i.e. it genuinely started new work. Jitter between two
					// active phases (thinking<->tool) or a stale echo of the
					// same active phase must not re-show a dismissed dialog.
					if (prev && prev !== ss.phase && isActivePhase(ss.phase) && !isActivePhase(prev)) {
						changed.push(ss.id)
					}
				})
			...
			react.useEffect(() => {
				...
				const url = '/petcenter/' + encodeURIComponent(active.dir) + '/spritesheet.webp?v=' + s.rev
				const img = new Image()
				img.onload = () => {
					if (!alive) return
			...
			if (!s.visible) {
				return el('div', { className: 'dsh-pc-summon', onClick: () => store.set({ visible: true }) }, '🐾 召唤桌宠')
			}
			...
			const dialogEls = shown.map((ss, idx) => {
				...
				return el('div', {
					key: ss.id,
					className: 'dsh-pc-dialog' + (isCollapsed ? ' collapsed' : ''),
					...
					el('div', { className: 'dsh-pc-dialog-body' },
						el('div', { className: 'dsh-pc-dialog-project' }, ss.project || 'DSH'),
						!isCollapsed ? el('div', { className: 'dsh-pc-dialog-doing' }, activity) : null
					),
					hovered && canStop
						? el('button', { className: 'dsh-pc-dialog-stop', title: '终止对话', onClick: (e) => { e.stopPropagation(); onStop(ss.id) } }, '⏹')
						: (statusIcon ? el('div', { className: 'dsh-pc-dialog-status ' + statusCls }, statusIcon) : null)
				)
			})
			...
			return el('div', {
				className: 'dsh-pc-pet-wrap' + (s.clickThrough ? ' dsh-pc-through' : ''),
				style: { left: Math.round(pos.x), top: Math.round(pos.y), zIndex: wrapZ, height: wrapH }
			},
				el('img', {
					ref: imgRef,
					className: 'dsh-pc-pet' + (dragging ? ' dragging' : ''),
					alt: '',
					draggable: false,
					width: frames.w,
					height: frames.h,
				...
				dialogEls,
				menu ? el('div', { className: 'dsh-pc-menu', style: { left: Math.round(menu.x), top: Math.round(menu.y) } },
					el('div', { className: 'dsh-pc-title' }, String(active.displayName || active.id)),
		...
		function setMode(next) {
			...
			rpc('mode', { mode: next }).then((r) => {
				if (r && r.ok) {
					...
					store.set({ busy: false, error: '桌面版未安装：请先在 dsh-codex-petcenter/desktop 目录执行 npm install 安装 Electron（' + (r.desktopDir || '') + '）' })
				} else {
		...
		function installSkin(id) {
			store.set({ installing: id, error: '' })
			return rpc('install', { id }).then((r) => {
				if (r && r.ok) {
					const pets = Array.isArray(r.pets) ? r.pets : []
					const active = pets.some((p) => p.dir === id)
						? id
						: (store.active && pets.some((p) => p.dir === store.active) ? store.active : (pets[0] ? pets[0].dir : null))
					store.set({ pets, rev: r.rev || 0, active, installing: null, bubble: '已安装' })
				} else {
					store.set({ installing: null, error: String((r && r.error) || '安装失败') })
				}
				return r
			}).catch((e) => {
				store.set({ installing: null, error: String((e && e.message) || e) })
			})
		}

		function uninstallSkin(id) {
			store.set({ busy: true, error: '' })
			return rpc('uninstall', { id }).then((r) => {
				if (r && r.ok) {
					const pets = Array.isArray(r.pets) ? r.pets : []
					const active = pets.some((p) => p.dir === store.active) ? store.active : (pets[0] ? pets[0].dir : null)
					store.set({ pets, rev: r.rev || 0, active, busy: false, bubble: '已卸载' })
				} else {
					store.set({ busy: false, error: String((r && r.error) || '卸载失败') })
				}
				return r
			}).catch((e) => {
				store.set({ busy: false, error: String((e && e.message) || e) })
		...
		function sourceBadge(site) {
			if (site === '内置') return el('span', { className: 'desc', style: { color: '#9ecbff' } }, '内置')
			if (site === 'codex-pet.org') return el('span', { className: 'desc', style: { color: '#7dd3fc' } }, '官网')
			if (site === 'github') return el('span', { className: 'desc', style: { color: '#86efac' } }, 'GitHub')
			return null
		}

		function refreshMarket() {
			store.set({ busy: true, error: '' })
			rpcGet('catalog?refresh=1').then((d) => {
				store.set({
					catalog: Array.isArray(d.items) ? d.items : [],
					catalogTags: Array.isArray(d.tags) ? d.tags : [],
					catalogSources: Array.isArray(d.sources) ? d.sources : [],
					catalogTotal: d.total || 0,
					busy: false
				})
			}).catch((e) => {
				store.set({ busy: false, error: String((e && e.message) || e) })
		...
		function PetControls(props) {
			...
			const filtered = (s.catalog || []).filter((c) => {
				if (s.tagFilter && s.tagFilter !== '全部' && !(c.tags || []).includes(s.tagFilter)) return false
				if (q && !((c.name || '').toLowerCase().includes(q) || (c.desc || '').toLowerCase().includes(q) || (c.id || '').toLowerCase().includes(q))) return false
				return true
			})
			return [
				el('div', { className: 'row', key: 'r0' },
					...
					el('label', { style: { cursor: 'pointer' } },
						el('input', {
							type: 'radio', name: 'dsh-pc-mode', checked: s.mode !== 'desktop',
				...
				el('div', { className: 'row', key: 'r0b' },
					el('button', { onClick: () => refresh(timer), disabled: s.busy }, s.busy ? '扫描中…' : '重新扫描 Codex 皮肤'),
					el('button', { onClick: () => store.set({ visible: !s.visible }) }, s.visible ? '隐藏桌宠' : '显示桌宠')
				),
				el('div', { className: 'dsh-pc-title', key: 'r1t' }, '已安装皮肤（' + (s.pets ? s.pets.length : 0) + '）'),
				s.pets.length === 0
					? el('div', { className: 'muted', key: 'r1e' }, '还没有皮肤，从下方「皮肤市场」下载一个吧。')
					: el('div', { className: 'pets', key: 'r1' },
						s.pets.map((p) => el('div', { key: p.dir, style: { display: 'flex', alignItems: 'center', gap: 8 } },
							el('label', { style: { cursor: 'pointer', flex: 1 } },
								el('input', {
									type: 'radio', name: 'dsh-pc-pet', checked: s.active === p.dir,
									onChange: () => { store.set({ active: p.dir }); save() }
								}),
								' ' + String(p.displayName || p.id),
								s.active === p.dir ? el('span', { className: 'desc' }, '（使用中）') : null,
								p.description ? el('span', { className: 'desc' }, ' — ' + String(p.description)) : null
							),
							el('button', { onClick: () => uninstallSkin(p.dir), disabled: s.busy, title: '卸载 ' + p.dir }, '卸载')
						))
					),
				el('div', { className: 'dsh-pc-title', key: 'r2t' }, '皮肤市场' + (s.catalogTotal ? '（' + s.catalogTotal + ' 款）' : '')),
				el('div', { className: 'row', key: 'r2s' },
					el('span', { className: 'muted' }, (s.catalogSources || []).join(' · ')),
					el('button', { onClick: refreshMarket, disabled: s.busy }, '刷新市场')
				),
				el('div', { className: 'row', key: 'r2' },
					el('input', {
						type: 'search', placeholder: '搜索皮肤名称/描述…', value: s.search,
						onChange: (e) => store.set({ search: e.target.value }),
						style: { flex: 1, minWidth: 140, background: 'rgba(127,127,127,.14)', border: '1px solid rgba(127,127,127,.3)', color: 'inherit', borderRadius: 8, padding: '4px 10px' }
					})
				),
				el('div', { className: 'row', key: 'r2b' },
					['全部'].concat(s.catalogTags || []).map((t) =>
						el('button', {
							key: t,
							onClick: () => store.set({ tagFilter: t }),
							style: s.tagFilter === t ? { background: 'rgba(127,127,127,.42)' } : null
						}, t)
					)
				),
				filtered.length === 0
					? el('div', { className: 'muted', key: 'r3' }, '没有匹配的皮肤。')
					: el('div', { className: 'pets', key: 'r3' },
						filtered.slice(0, showCount).map((c) => el('div', { key: c.id, style: { display: 'flex', alignItems: 'center', gap: 8 } },
							el('div', { style: { flex: 1 } },
								el('span', { style: { fontWeight: 600 } }, c.name || c.id),
								' ',
								sourceBadge(c.site),
								c.creator ? el('span', { className: 'desc' }, ' · ' + c.creator) : null,
								c.tags && c.tags.length ? el('span', { className: 'desc' }, ' [' + c.tags.join(' / ') + ']') : null,
								c.desc ? el('div', { className: 'muted' }, c.desc) : null
							),
							c.installed
								? el('span', { className: 'desc' }, '已安装')
								: el('button', {
									onClick: () => installSkin(c.id),
									disabled: s.installing !== null
								}, s.installing === c.id ? '下载中…' : '下载')
						))
					),
				filtered.length > showCount
					? el('div', { className: 'row', key: 'r3b' },
						el('button', { onClick: () => setShowCount(showCount + 60) }, '加载更多（剩余 ' + (filtered.length - showCount) + ' 款）')
					)
					: null,
				el('div', { className: 'row', key: 'r5' },
					'缩放 ',
					el('input', {
		...
		function PetPanel(props) {
			const s = useStore()
			return el('div', { className: 'dsh-pc-panel' },
				el('div', { className: 'dsh-pc-title' }, 'Codex 桌宠 · DSH'),
		...
		function PetSettingsPage(props) {
			const s = useStore()
			return el('div', { className: 'dsh-pc-panel' },
				el('div', { className: 'dsh-pc-title' }, '桌宠皮肤中心'),
				el('div', { className: 'muted' }, '浏览、下载、卸载 Codex 桌宠皮肤并随时切换；皮肤来自开源社区仓库（jsDelivr CDN 分发），配置持久化到 $DSH_HOME/pet.json。'),
				PetControls({ s, timer: props.timer }),
		...
		function apply(ctx) {
			...
			// Load the skin catalog (market)
			rpcGet('catalog').then((d) => {
				store.set({
					catalog: Array.isArray(d.items) ? d.items : [],
					catalogTags: Array.isArray(d.tags) ? d.tags : [],
					catalogSources: Array.isArray(d.sources) ? d.sources : [],
			...
			slots.inject('shell.overlay', () => slots.register(
				{ name: 'shell.overlay', id: 'petcenter-overlay', order: 100 },
			...
			slots.inject('settings.section', () => slots.register(
				{ name: 'settings.section', id: 'petcenter', order: 18, label: '桌宠皮肤中心' },
		...
		exports.name = 'dsh-codex-petcenter'
